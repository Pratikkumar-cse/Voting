...
contract UserToken is MembershipToken {
    ...
    function _burn(address user, uint256 _id, uint256 _value) onlyOwner public {
        memberCheck[user] = false;
        info[user].id = 0;
        info[user].MemberPaid = false;
        info[user].MemberKYC = false;
        info[user].Certifier = false;
        info[user].Champion = false;
        super._burn(user, _id, _value);
    }

    function certifierGetter(address user) public view returns(bool) {
        return info[user].Certifier;
    }
    
    function ChampionGetter(address user) public view returns(bool) {
        return info[user].Champion;
    }
    
    function memberId(address user) public view returns(uint) {
        return info[user].id;
    }
    ...
    function certifierUpdate(address user) public {
